...
### First, check if the necessary data is available in the DataFrame. For this analysis, we will assume that the data is all available since we are using a sample dataset. To check if data is available in the running Shiny App, we use an if statement that looks like the following : if (all(c("Race", "Gender", "UseOfForce) %in% colnames(policingdata))

### For the Q11, the race, use of force, and gender variables must be available to answer the question. If there is missing data in these columns, filter them out. 

### For this situation, we need to calculate the number of incidents where there is use of force in dplyr. 

### Question: Are officers more likely to use force on subjects with specific intersectional identities?
...
```{r}
p <- qdata %>%
  ggplot(aes(x = proportion, y = Race)) +
  geom_col(aes(fill=Gender), position = "dodge", width=0.8) +
  ylab("Race") +
  xlab("Proportion of Incidents") +
